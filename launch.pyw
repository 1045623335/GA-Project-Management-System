...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-wechat', action='store_true', help='禁用自动启动微信个人 Bot');
    ...
    if args.no_wechat:
        print('[Launch] WeChat Bot auto-start disabled (--no-wechat)')
    else:
        wechatproc = subprocess.Popen([sys.executable, os.path.join(frontends_dir, "wechatapp.py")], creationflags=subprocess.CREATE_NO_WINDOW if os.name=='nt' else 0)
        atexit.register(wechatproc.kill)
        print('[Launch] WeChat Bot auto-start requested (duplicate prevented by wechatapp lock)')
